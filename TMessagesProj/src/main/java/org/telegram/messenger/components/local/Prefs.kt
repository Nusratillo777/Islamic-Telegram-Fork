...
object Prefs {
    ...
    private var mPrayTimes: String
        get() = preferences.getString("mPrayTimes", "") ?: ""
        set(value) = preferences.edit { it.putString("mPrayTimes", value) }
    ...
    fun getPrayTimes(): PrayTimes? {
        return try {
            if (mPrayTimes.isNotBlank())
                Gson().fromJson(mPrayTimes, PrayTimes::class.java)
            else null
        } catch (e: Exception) {
            Log.e("TTT", e.toString())
            null
    ...
    fun setPrayTimes(
        prayTimes: Map<String, String>,
        today: String,
        prayerTimesTomorrow: Map<String, String>,
        tomorrow: String,
        prayerTimesAfterTomorrow: Map<String, String>,
        afterTomorrow: String
    ){
        try {
            val list= mutableListOf<PrayTime>()
            for ((prayer, time) in prayTimes) {
                list.add(PrayTime(
                    time = time,
                    name = prayer,
                    day = today
            ...
            for ((prayer, time) in prayerTimesTomorrow) {
                list.add(PrayTime(
                    time = time,
                    name = prayer,
                    day = tomorrow
            ...
            for ((prayer, time) in prayerTimesAfterTomorrow) {
                list.add(PrayTime(
                    time = time,
                    name = prayer,
                    day = afterTomorrow
            ...
            mPrayTimes = Gson().toJson(PrayTimes(list)).toString()
            Log.e("TTT", mPrayTimes)
        } catch (e: Exception) {
            Log.e("TTT", e.toString())
        }
    }
