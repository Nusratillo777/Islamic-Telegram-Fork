...
class PrayChangeTimeActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_pray_change_time)
...
//            addView(textCell)
            val textCell1 = TextCell(this@PrayChangeTimeActivity).apply {
                ...
                setBackgroundColor(Theme.getColor(Theme.key_windowBackgroundWhite))
                setText("$name: $selected min", true)
            }
            addView(textCell1)
            setOnClickListener {
                showNumberPickerDialog(selected){number->
                    textCell1.setText("$name: $number min",true)
                    block(number)
                }
            }
    ...
    private fun showNumberPickerDialog(selected:Int,block:(Int)->Unit) {
        ...
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("OK") { dialog, _ ->
                selectedNumber = numberPicker.value - 100
