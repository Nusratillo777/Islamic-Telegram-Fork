...
import org.telegram.messenger.components.notifications.LocationService
import org.telegram.messenger.components.permissions.checkPermissionLocation
import org.telegram.messenger.components.notifications.checkAlarmAccess
...
class PrayTimesView @JvmOverloads constructor(
    ...
    private val bounceView:ImageView
    private val lineView:ImageView
    ...
    init {
        ...
        bounceView = findViewById(R.id.viewBounce)
    ...
    fun update(){
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            val afterTomorrow = SimpleDateFormat("dd-MM-yyyy", Locale.getDefault()).format(Calendar.getInstance().apply { add(Calendar.DAY_OF_YEAR, 2) }.time)
            ...
            val tune = "0,${Prefs.addTimeFajr},${Prefs.addTimeMorning},${Prefs.addTimeDhuhur},${Prefs.addTimeAsr},${Prefs.addTimeMaghrib},0,${Prefs.addTimeIsha},0"
            ...
            val afterTomorrowResponse = getJsonResponse("https://api.aladhan.com/v1/timings/$afterTomorrow?latitude=$latitude&longitude=$longitude&method=$methodId&tune=$tune")
            ...
            todayResponse?.let {todayResult->
                tomorrowResponse?.let {tomorrowResult->
                    yesterdayResponse?.let {yesterdayResult->
                        afterTomorrowResponse?.let {afterTomorrowResult->
                            // Handle the JSON response on the main thread
                            withContext(Dispatchers.Main) {
                                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                                    handleJsonResponse(todayResponse,tomorrowResponse,yesterdayResponse,afterTomorrowResponse,today,tomorrow,yesterday,afterTomorrow)
                                }
                            }
                        }
                    }
    ...
    fun updateTimes(
        ...
        val ishaYesterdayTime = LocalTime.parse(fajr)
        ...
        val fajrTomorrowTime = LocalTime.parse(fajrTomorrow)
        when {
            currentTime.isBefore(fajrTime) -> {
                val differHour = currentTime.hour + (23-ishaYesterdayTime.hour)
                val differMinute = currentTime.minute + (60-ishaYesterdayTime.minute)
                val differ = differHour*60 + differMinute

                val differHour1=fajrTime.hour + (24-ishaYesterdayTime.hour)
                val differMinute1=fajrTime.minute + (60-ishaYesterdayTime.minute)
                val differ1 = differHour1*60+differMinute1
                val percent = differ.toFloat()/differ1

                bounceView.x = lineView.width*percent
            ...
            currentTime.isBefore(sunriseTime) -> {
                val differHour = currentTime.hour - (fajrTime.hour)
                val differMinute = currentTime.minute - (fajrTime.minute)
                val differ = differHour*60 + differMinute

                val differHour1=sunriseTime.hour - (fajrTime.hour)
                val differMinute1=sunriseTime.minute - (fajrTime.minute)
                val differ1 = differHour1*60+differMinute1
                val percent = differ.toFloat()/differ1
            ...
            currentTime.isBefore(dhuhrTime) -> {
                val differHour = currentTime.hour - (sunriseTime.hour)
                val differMinute = currentTime.minute - (sunriseTime.minute)
                val differ = differHour*60 + differMinute

                val differHour1=dhuhrTime.hour - (sunriseTime.hour)
                val differMinute1=dhuhrTime.minute - (sunriseTime.minute)
                val differ1 = differHour1*60+differMinute1
                val percent = differ.toFloat()/differ1
            ...
            currentTime.isBefore(asrTime) -> {
                val differHour = currentTime.hour - (dhuhrTime.hour)
                val differMinute = currentTime.minute - (dhuhrTime.minute)
                val differ = differHour*60 + differMinute

                val differHour1=asrTime.hour - (dhuhrTime.hour)
                val differMinute1=asrTime.minute - (dhuhrTime.minute)
                val differ1 = differHour1*60+differMinute1
                val percent = differ.toFloat()/differ1
            ...
            currentTime.isBefore(maghribTime) -> {
                val differHour = currentTime.hour - (asrTime.hour)
                val differMinute = currentTime.minute - (asrTime.minute)
                val differ = differHour*60 + differMinute

                val differHour1=maghribTime.hour - (asrTime.hour)
                val differMinute1=maghribTime.minute - (asrTime.minute)
                val differ1 = differHour1*60+differMinute1
                val percent = differ.toFloat()/differ1
            ...
            currentTime.isBefore(ishaTime) -> {
                val differHour = currentTime.hour - (maghribTime.hour)
                val differMinute = currentTime.minute - (maghribTime.minute)
                val differ = differHour*60 + differMinute

                val differHour1=ishaTime.hour - (maghribTime.hour)
                val differMinute1=ishaTime.minute - (maghribTime.minute)
                val differ1 = differHour1*60+differMinute1
                val percent = differ.toFloat()/differ1
                ...
                val differHour = currentTime.hour - (ishaTime.hour)
                val differMinute = currentTime.minute - (ishaTime.minute)
                val differ = differHour*60 + differMinute

                val differHour1=fajrTomorrowTime.hour - (ishaTime.hour)
                val differMinute1=fajrTomorrowTime.minute - (ishaTime.minute)
                val differ1 = differHour1*60+differMinute1
                val percent = differ.toFloat()/differ1
    ...
    @RequiresApi(Build.VERSION_CODES.O)
    private fun handleJsonResponse(todayResult: String, tomorrowResult: String, yesterdayResult:String, afterTomorrowResult:String, today:String, tomorrow:String, yesterday:String, afterTomorrow:String) {
        try {
            ...
            val afterTomorrowResponse = JSONObject(afterTomorrowResult).getJSONObject("data").getJSONObject("timings")
            ...
            val prayerTimesAfterTomorrow = mapOf(
                "Fajr" to afterTomorrowResponse.getString("Fajr"),
                "Sunrise" to afterTomorrowResponse.getString("Sunrise"),
                "Dhuhr" to afterTomorrowResponse.getString("Dhuhr"),
                "Asr" to afterTomorrowResponse.getString("Asr"),
                "Maghrib" to afterTomorrowResponse.getString("Maghrib"),
                "Isha" to afterTomorrowResponse.getString("Isha"),
            )
            updateTimes(fajr,sunrise,dhuhr,asr,maghrib,isha,fajrTomorrow,ishaYesterday)
            // Schedule alarms for the prayer times
//            val currentTime = LocalTime.now()
//            val prayerTimesToday1 = mapOf(
//                "Fajr12" to "${currentTime.hour}:${currentTime.minute+1}",
//                "Fajr1" to "${currentTime.hour}:${currentTime.minute+2}",
//                "Fajr" to "${currentTime.hour}:${currentTime.minute+3}",
//                "Sunrise" to "${currentTime.hour}:${currentTime.minute+4}",
//                "Dhuhr" to "${currentTime.hour}:${currentTime.minute+5}",
//                "Asr" to "${currentTime.hour}:${currentTime.minute+6}",
//                "Maghrib" to "${currentTime.hour}:${currentTime.minute+7}",
//                "Isha" to "${currentTime.hour}:${currentTime.minute+8}",
//                "Isha1" to "${currentTime.hour}:${currentTime.minute+9}",
//                "Isha2" to "${currentTime.hour}:${currentTime.minute+10}",
//            )
            Prefs.setPrayTimes(prayerTimesToday, today,prayerTimesTomorrow, tomorrow,prayerTimesAfterTomorrow, afterTomorrow)
        } catch (e: Exception) {
            Log.e("TTT",e.toString())
            e.printStackTrace()
        }
    ...
    fun onResume(activity: Activity) {
        blockTry{
            update()
            if (isFirst){
                startServiceLocation(activity)
                isFirst = false
            }
        }
    }

    private fun startServiceLocation(activity: Activity?){
